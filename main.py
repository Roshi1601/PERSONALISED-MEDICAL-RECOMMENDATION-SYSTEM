...
prescription = pd.read_csv("datasets/prescription.csv")

# Load model
svc = pickle.load(open('models/svc.pkl','rb'))

# Helper functions to fetch relevant disease data
def helper(dis):
...
# Mapping symptoms to indices (as per your dictionary)
symptoms_dict = {
...
def predict():
    if request.method == 'POST':
        ...
        if not symptoms:
            ...
            dis_des, precautions, medications, rec_diet, workout, prescription = helper(predicted_disease)

            # Format precautions into list for display
            my_precautions = []
